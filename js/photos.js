document.addEventListener("DOMContentLoaded", () => {
  ...
  const imageFilenames = [
    "Amsterdam Central Canal.JPG",
    "Amsterdam from Restaurant.JPG",
    "Amsterdam Train Station.jpg",
    "Auditorium.jpg",
    "Bolzano.jpg",
    "Cathedral.JPG",
    "Dougie.png",
  ...
  prevButton.addEventListener("click", () => {
    currentIndex =
      currentIndex > 0 ? currentIndex - 1 : imageFilenames.length - 1; // Loop to last
    slideshowImage.src = `images/${imageFilenames[currentIndex]}`; // Update image
  });

  nextButton.addEventListener("click", () => {
    currentIndex =
      currentIndex < imageFilenames.length - 1 ? currentIndex + 1 : 0; // Loop to first
    slideshowImage.src = `images/${imageFilenames[currentIndex]}`; // Update image
  });
